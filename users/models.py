...
from utils.models import LifeCycle, LifeCycleDescription
...
class User(AbstractBaseUser, PermissionsMixin):
    groups = models.ManyToManyField('auth.Group', verbose_name=('groups'), blank=True, help_text=('The groups this user belongs to. A user will get all permissions ' 'granted to each of their groups.'),
    ...
    user_name = models.CharField(('user_name'), max_length=150, unique=True, help_text=('Requeridos. 150 caracteres ou menos. Apenas letras, dígitos e @/./+/-/_.'), error_messages={ 'unique': ("Um usuário com esse nome já existe."),},)

    full_name = models.CharField(("Nome completo"), max_length=255)
    phone_number = models.CharField(("Celular"), max_length=255)
    email = models.EmailField(('email address'), blank=True, null=True, unique=True, error_messages={'unique': ("Um usuário com este email já existe")})
    administrator = models.BooleanField(("Administrador"), default=False)
    jury = models.BooleanField(("Administrador"), default=False)

    is_staff = models.BooleanField(('staff status'), default=False, help_text=('Designa se o usuário pode fazer login neste site de administração.'),)
    is_active = models.BooleanField(('active'), default=True, help_text=('Designa se este usuário deve ser tratado como ativo.' 'Desmarque isso em vez de excluir contas.'),)
    ...
    @property
    def token(self):
        token = jwt.encode({'user_name': self.user_name, 'email': self.email, 'exp': datetime.utcnow() + timedelta(hours=24)}, settings.SECRET_KEY, algorithm='HS256')
        return token

    def __str__(self):
        return '{}, {}'.format(self.user_name, self.email)

class Cidadao(LifeCycle):
    full_name = models.CharField(("Nome completo"), max_length=255)
...
class FuncionarioJudicial(LifeCycle):
...
class Permission(LifeCycleDescription):
    def __str__(self):
        return f"{self.description}"

class UserPermissions(LifeCycle):
    utilizador = models.ForeignKey(User, on_delete=models.CASCADE, related_name='permissoes')
    role = models.ForeignKey(Permission, on_delete=models.CASCADE, null=True, blank=True)

    def __str__(self):
        return f"{self.utilizador.full_name} - {self.role}"